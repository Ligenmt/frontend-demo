class DepthPaint {
    ...
    setRenderData(info) {
        ...
        if (this._leftPointsArr.length == 0) {
            // 初始点
            this._leftPointsArr.push({x: info.currentX - info.r / 2 / info.stepX, y: info.paintY, cos: info.cos});
            this._rightPointsArr.push({x: info.currentX + info.r / 2 / info.stepX, y: info.paintY, cos: info.cos});
        } else {
            if (info.prevR !== info.r) {
                this._leftPointsArr.push({x: x1, y: y1, cos: info.cos});
                this._rightPointsArr.push({x: x2, y: y2, cos: info.cos});
            }
            if (info.cos !== info.prevCos) {
                ...
                this._leftPointsArr.push({x: x3, y: y3, cos:info.cos});
                this._rightPointsArr.push({x: x4, y: y4, cos:info.cos});
            }
        }
    ...
    renderPaint(ctx) {
        let leftLen = this._leftPointsArr.length;
        let rightLen = this._rightPointsArr.length;
        ...
        this._paint(ctx, {
            x1: this._leftPointsArr[leftLen - 1].x,
            y1: this._leftPointsArr[leftLen - 1].y,
            x2: this._rightPointsArr[rightLen - 1].x,
            y2: this._rightPointsArr[rightLen - 1].y,
            cos: this._leftPointsArr[leftLen - 1].cos,
        });
    }
